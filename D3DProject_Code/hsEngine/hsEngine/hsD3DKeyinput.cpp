...
bool Keyinput::init(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
...
bool Keyinput::DownKey(unsigned char state)
{
	if (m_keyboardState[state] & 0x80)
	{
		return true;
	}
...
bool Keyinput::UpKey(unsigned char state)
{
	if (m_keyboardState[state] & 0x80)
	{
		return true;
...
void Keyinput::GetMouseLocation(int& mouseX, int& mouseY)
{
	mouseX = m_mouseX;
	mouseY = m_mouseY;
}