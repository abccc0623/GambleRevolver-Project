...
public:
	...
	ASEParser::Scenedata m_scenedata;			// 현재 Scene의 데이터가 들어갈 구조체

	/// 매트리얼 관련
	int	m_materialcount;					// Material이 총 몇개인가?
	ASEParser::ASEMaterial* m_materialdata;			// Material의 데이터가 들어갈 구조체
	list<ASEParser::ASEMaterial*> m_list_materialdata;	// Material들의 리스트

	/// 한 씬에 N개의 메시가 존재할 수 있다.
	ASEParser::Mesh* m_OneMesh;						// 메시 하나의 데이터가 들어갈 구조체
	std::vector<ASEParser::Mesh*> m_MeshList;				// 메시들
	std::vector<ASEParser::Mesh*> m_HELPER_MeshList;				// 메시들

	/// 쉐이프 오브젝트를 파싱할 때 필요한 중간 변수
	//( 파서 내부에서 쉐이프 오브젝트는 GeomObject와 동일하게 취급된다)
	ASEParser::ShapeLine* m_nowshapeline;				// 현재 읽고 있는 Line
	ASEParser::ShapeVertex* m_nowshapevertex;			// 현재 읽고 있는 ShapeVertex

	/// 애니메이션
	hsAnimation* m_animation;					// 한 오브젝트의 애니메이션 데이터(파싱용)
	list<hsAnimation*> m_list_animation;		// 그 데이터의 리스트
...
private:
	...
	///스플릿을 한다
	void Split();
	///텍스쳐 정보를 가지고있는 놈의 스플릿
	void On_TextureSplit(int& AddCount);
	///텍스쳐 정보를 가지고있지 않은놈의 스플릿
