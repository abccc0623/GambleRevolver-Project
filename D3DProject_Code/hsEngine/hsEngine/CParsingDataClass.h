...
namespace ASEParser
{
	...
	public:
		...
		bool					m_is_boneObject;

		int BoneNumber;
		string BoneName;
		std::map<int,string>m_is_BoneList;
		int Weightindex = 0;
		Bone* m_bone;									// Bone 한개의 포인터 (파서 내부에서 임시 공간으로 쓰임)
		VertexWeight* m_wvertex = nullptr;						// Weight_Vertex 한개의 포인터 (마찬가지로 임시공간)
